        ...
        if (settings.output.timestampOpts.utc === true) {
            if (typeof data.timestamp !== "undefined") {
                if (typeof settings.timeformat !== "undefined" && settings.timeformat !== "undefined") {
                    try {
                        f = moment.utc().format(settings.timeformat);
                    } catch (e) {
                        f = moment.utc().format('DD MMM HH:mm:ss');
                    }
                } else {
                    f = formatTimestamp(data.timestamp, true);
                }
            } else {
                f = formatTimestamp(new Date(), true);
            }
        } else {
            if (typeof data.timestamp !== "undefined") {
                if (typeof settings.timeformat !== "undefined" && settings.timeformat !== "undefined") {
                    ...
                    f = formatTimestamp(data.timestamp, false);
                }
            } else {
                f = formatTimestamp(new Date(), false);
            }
        }
