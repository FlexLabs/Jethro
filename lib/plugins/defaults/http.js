...
HttpServer.prototype.format = function(data, callback) {
    var ip = this.spaceOut((data.ip.substr(0, 3) == "::1" ? "127.0.0.1" : data.ip.substr(0, 7) === "::ffff:" ? data.ip.substr(7) : data.ip), 18
    );
    ...
    var responseTime = data.responseTime < 50 ? forceChalk.green.bold(data.responseTime + " ms") : data.responseTime < 250 ? forceChalk.yellow.bold(data.responseTime + " ms") : forceChalk.red.bold(data.responseTime + " ms");
